    ...
    & {
      [Key in `${Union[Tag]}`]: (
        value: Extract<
          Union,
          {
            [K in Tag]: Key extends "true" ? true
              : Key extends "false" ? false
              : Key;
          }
        >,
      ) => any;
 ...
 */
export type ValidateKeys<T, U> = [keyof T] extends [keyof U] ? T
  : {
...
export const corrupt = (unreachable: never): never => {
  ...
  throw new TypeError(
    `Internal Error: encountered impossible value "${
      safeStringify(
        unreachable,
      )
    }"`,
